...
class FlowAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None):
        ...
        normalizer_row = 1.0 / (
            torch.einsum(
                "nhld,nhd->nhl", queries + 1e-6, keys.sum(dim=2) + 1e-6
            )
        )
        normalizer_col = 1.0 / (
            torch.einsum(
                "nhsd,nhd->nhs", keys + 1e-6, queries.sum(dim=2) + 1e-6
            )
        )
        # reweighting
        ...
        normalizer_row_refine = torch.sigmoid(
            ...
            * (float(queries.shape[2]) / float(keys.shape[2]))
        )
        normalizer_col_refine = (
            torch.softmax(normalizer_col_refine, dim=-1) * keys.shape[2]
        )  # B h L vis
        # multiply
        kv = keys.transpose(-2, -1) @ (
            values * normalizer_col_refine[:, :, :, None]
        )
        x = (
            (
...
class FullAttention(nn.Module):
    ...
    def forward(
        self, queries, keys, values, padding_mask, tau=None, delta=None
    ):
...
class ProbAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None):
        ...
        u = (
            self.factor * np.ceil(np.log(L_Q)).astype("int").item()
        )  # c*ln(L_q)
